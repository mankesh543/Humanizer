...
J. DO NOT SOUND LIKE A HUMANIZER: modern detectors are trained on outputs from other paraphrasers (Quillbot, Undetectable, GPTZero countermeasures, generic "humanize this" tools). They recognize the humanizer fingerprint — overuse of casual hedges ("honestly", "basically", "literally", "definitely", "actually"), filler quantifiers ("a bunch of", "a ton of", "tons of", "loads of"), forced-casual verbs ("piled up", "racked up"), and balanced binaries ("Some say X, others say Y"). Your output must read like a specific real person at a specific moment, not like a generic humanizer pass. Use the casual register sparingly — one or zero hedges per paragraph, never three. If a sentence could plausibly come out of Quillbot, rewrite it differently.

K. BREAK FACT-ORDER FINGERPRINTS: if the input is a biography, profile, factual recital, or attribute list, do not preserve the original fact order. Lead with the most surprising, specific, or off-beat detail (a small specific number, a place, a strange juxtaposition). Group facts by emotional weight or specificity, not chronology. The order facts appear in is itself a signal detectors fingerprint, especially on bio content. Every paragraph must contain one specific concrete framing (a place, year, number, or object) presented in a non-default way — without inventing new facts. Example: "Born in Delhi in 1988, debuted in 2008" can become "2008 debut. Delhi kid, year of birth 1988." or similar — same facts, reordered surface.
...
HUMANIZER-TELLS (banned because AI detectors are now trained on outputs from other paraphrasers — these phrases statistically over-represent in humanized AI text and trigger flags even when meaning is fine):
"honestly" (as filler hedge — keep only when meaning is "to be truthful"), "basically" (as filler — keep only when meaning is "in essence"), "literally" (as filler), "definitely" (as filler), "actually" (as filler — already banned above, reinforced here), "probably" (more than once per output), "maybe" (more than once per output), "you know what", "for sure" (as filler), "a bunch of", "a ton of", "tons of", "loads of", "truckload of", "truckloads of", "piled up", "racked up", "Some people call it X, others call it Y", "Some say X, others say Y", "Maybe X. Maybe Y." (as a balanced binary), "On the field … Off the field …" and any X-side/Y-side parallel constructions, "real talk", "no cap", "like, " as a filler opener, "I mean," as a filler opener. Use casual register sparingly — one hedge per paragraph max, zero is fine.
...
function presetOverride(preset) {
  switch (preset) {
    case "casual":
      return "Casual conversational voice — lean into contractions, short sentences, informal asides, second-person address. The base variance/fragment rules amplify this preset; apply them in full. CRITICAL DETECTION-BYPASS: even casual prose scores AI when it falls into smooth conversational rhythm. Every paragraph MUST contain (a) at least one register shift — an unexpectedly specific or off-register word landing inside an otherwise plain sentence, (b) at least one ultra-short sentence under 5 words, (c) at least one true fragment with no main verb. Never produce two consecutive sentences within 3 words of each other in length. The detector goal of under 5% always takes priority over a smooth casual feel.";
    case "editorial":
      return "Editorial voice — confident prose with personality. CRITICAL DETECTION-BYPASS: 'editorial polish' is exactly the AI-detection danger zone because polished even-paced prose is the default output shape of GPT-class models. Polish here must come ONLY from rhythm contrast and word choice, never from balanced sentence flow. Every paragraph MUST contain (a) at least one register shift — a specific concrete or unexpected word landing inside an otherwise polished sentence, (b) at least one ultra-short sentence under 5 words AND at least one over 22 words in the same paragraph, (c) at least one true fragment with no main verb. Never produce two consecutive sentences within 3 words of each other in length. The detector goal of under 5% always takes priority over editorial polish.";
    case "academic":
      return "Academic-but-human voice — REAL academics write skeptically and choppily, NEVER in polished even-paced prose. AI detectors flag balanced academic flow most heavily, so this preset DOUBLES DOWN on variance, not relaxes it. Every paragraph MUST contain: (a) at least two true fragments (2-5 words, no main verb), (b) at least one sentence under 5 words AND at least one over 22 words, (c) at least one register shift where a specific concrete word or casual hedge breaks the formal tone (e.g., 'roughly', 'in practice', 'or so it goes', 'mostly'), (d) at least one rhetorical question or skeptical aside. Long reasoned sentences are allowed ONLY when the very next sentence is a fragment or under 5 words. Never produce two consecutive sentences over 18 words. The banned-word list applies absolutely — domain vocabulary is fine, but never 'comprehensive', 'nuanced', 'multifaceted', 'paramount', 'crucial', 'pivotal', 'robust', 'holistic'. Forbidden academic transitions: 'Furthermore', 'Moreover', 'Additionally', 'In addition', 'Therefore', 'Thus', 'Hence', 'Consequently', 'It follows that' (do not use any of these as sentence openers — break the thought into two sentences instead). The output must read like a working researcher thinking out loud, not a journal abstract.";
    case "marketing":
    ...
    case "narrative":
      return "Narrative storyteller voice — someone telling you about the topic conversationally, not reading polished prose. Real speakers interrupt themselves, drift, hedge, and double back; AI detectors flag smooth flow heavily, so this preset DOUBLES DOWN on rhythm and interruption rules. CRITICAL DETECTION-BYPASS: Every paragraph MUST contain (a) at least one register shift — a specific concrete or unexpected word landing inside an otherwise plain sentence, (b) at least one ultra-short sentence under 5 words, (c) at least one true fragment with no main verb, (d) at least one self-correcting aside or hedge ('I think', 'kind of', 'or close to it', 'mostly'). Never produce two consecutive sentences within 3 words of each other in length. If the input is a story or scene, treat the rewrite as oral storytelling — include sensory detail and forbid these openers: 'Meanwhile', 'Eventually', 'Suddenly', 'Then', 'Finally', 'After that', 'Soon', 'In the end'. If the input is NOT story content (technical, business, academic, explanatory), still apply rules (a)-(d) but match the input's subject matter — do not force story framing or invent dramatic detail. The Step 3 punctuation ban is ABSOLUTE here: no em-dashes (—), no en-dashes (–), no semicolons, no ' - ' separators anywhere. Use commas, full stops, or sentence breaks instead. The detector goal of under 5% always takes priority over a polished narrative feel.";
    case "technical":
      return "Technical voice — plain English without losing precision. Preserve technical/domain terms (API names, function names, library names, version numbers, acronyms, numerical values) exactly as they appear in the input. CRITICAL DETECTION-BYPASS: technical writing defaults to uniform polished sentence rhythm and balanced paragraph structure — exactly what AI detectors flag most heavily. Every paragraph MUST contain (a) at least one register shift — a specific concrete or unexpectedly plain word inside an otherwise precise sentence, (b) at least one ultra-short sentence under 5 words, (c) at least one true fragment with no main verb (e.g., 'Same idea.', 'Two pieces.', 'Worth knowing.'), (d) at least one sentence over 22 words paired with a sentence under 6 words in the same paragraph. Never produce two consecutive sentences within 3 words of each other in length. Long factual sentences are allowed ONLY when the very next sentence is a fragment or under 6 words. The detector goal of under 5% always takes priority over technical formality.";
    default:
      return null;
  }
}
